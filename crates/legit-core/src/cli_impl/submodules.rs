...
impl<E: GitExecutor + ?Sized> GitCliBackend<E> {
    ...
    async fn submodule_gitdir_path(&self, name: &str) -> Result<Option<PathBuf>, GitError> {
        ...
        let out = runner
            ...
            .await?;
        Self::ensure_success(&out)?;
        // Textual '/' joins: the git dir is a host path (posix on a remote
        // host), never native-join material for the app OS.
        let modules = crate::fs::HostPath(out.stdout.trim().to_string()).join("modules");
        let gitdir = modules.join(name);
        let is_dir = matches!(
        ...
        // This path is deleted wholesale: it must resolve to exactly
        // `<real modules>/<name>`, so a symlink planted in the git dir cannot
        // redirect the deletion elsewhere (component-wise PathBuf comparison:
        // canonical forms may mix separators on Windows).
        let (Ok(real), Ok(real_modules)) = (
            self.fs.canonicalize(&gitdir).await,
            self.fs.canonicalize(&modules).await,
        ) else {
            return Err(GitError::Internal(format!(
                "could not resolve the gitdir of submodule '{name}'"
            )));
        };
        if real.as_local() != real_modules.as_local().join(name_path) {
            return Err(GitError::Internal(format!(
                "the gitdir of submodule '{name}' resolves outside {real_modules} - refusing to report or delete it"
            )));
        }
        Ok(Some(gitdir.as_local()))
    }
    ...
    pub(super) async fn submodules(&self) -> Result<Vec<SubmoduleInfo>, GitError> {
        ...
        // No gitlinks AND nothing declared -> no rows; skip the remaining
        // reads (wasted spawns for a known-empty answer). Declared-only
        // entries (`.gitmodules` written but the gitlink never `git add`ed)
        // must NOT hit this fast path: `assemble_submodules` surfaces them
        // as declared-but-never-added rows.
        if gitlinks.is_empty() && gitmodules.is_empty() {
            return Ok(Vec::new());
        }
