// Integration suite: the git backend against the real `git` binary in
// throwaway tempdir repositories. INCLUDED (via `include!`) by two thin test
// targets that each define `type BackendGuard` and
// `async fn make_backend(&Path) -> (Arc<dyn GitBackend>, BackendGuard)`:
// `legit-core/tests/git_flows.rs` (local `GitRunner`) and
// `legit-agent/tests/remote_git_flows.rs` (a `RemoteExecutor` speaking to a
// spawned agent — the remote gate: every flow, env-hardening, byte-safety,
// and cancellation assumption must hold identically over the wire).
//
...
impl TestRepo {
    async fn init() -> Self {
        ...
        let (backend, guard) = make_backend(&path).await;
        Self { _dir: dir, path, backend, _guard: guard }
    }
...
async fn superproject_path_resolves_from_inside_a_submodule() {
    ...
    let (sub_backend, _sub_guard) = make_backend(&sub_path).await;
