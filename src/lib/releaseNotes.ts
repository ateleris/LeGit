 ...
 * is filled by scripts/updater-json.mjs from the GitHub release body, which
 * release.yml assembles as: the tag's CHANGELOG.md section, a `---`
 * horizontal rule, then a static download/signing footer. Everything from the
 * first rule on is boilerplate - only the part before it belongs in the
 * update prompt.
 *
