// The Repo Settings sections, consumed through the repo settings manifest.
// Each edits repo-scope settings for one repository; scope and scope target
// are explicit per DESIGN-v0.2.md §F.6.
...
export function RepoGitExecutableSection({
  repoId,
  repoSettings,
}: {
  repoId: string;
  ...
  const apply = (path: string | null) =>
    runApply(async () => {
      ...
      setSuccessMsg(null);
      try {
        await api.setRepoGitPath(repoId, path);
        await refresh();
 ...
 */
export function RemoteRepoGitSection({ distro }: { distro: string }) {
...
export function ExternalEditorRepoSection({
  repoId,
  repoSettings,
}: {
  repoId: string;
...
export function LineEndingChangesRepoSection({
  repoId,
  repoSettings,
}: {
  repoId: string;
...
export function CommitTreeRepoSection({
  repoId,
  repoSettings,
}: {
  repoId: string;
...
export function SubmoduleAutoUpdateSection({
  repoId,
  repoSettings,
}: {
  repoId: string;
...
export function AutoPushTagsRepoSection({
  repoId,
  repoSettings,
}: {
  repoId: string;
...
export function LfsWarningRepoSection({
  repoId,
  repoSettings,
}: {
  repoId: string;
...
export function LineEndingsRepoSection({ repoId }: { repoId: string }) {
